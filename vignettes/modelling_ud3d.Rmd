---
title: "Modelling 3D utilization distributions from acoustic telemetry data in R"
...
2. **Simulation of synthetic path realizations: **A large number of stochastic trajectories are simulated from the acoustic telemetry data, taking into account the characterization of detection probabilities, the depth values, and the local topography.
...
The `syntPath` function generates synthetic path realizations by sampling geographic coordinates for each detection in the acoustic telemetry data, taking into account the following points:
...
We can plot the 3D-UD contours using the `rgl` package. This example shows the 50% and the 90% UD contours (inner and outer volumes) for the two individuals in the example dataset (`dentex18` in purple-blue and `dentex43` in yellow-orange).
...
Plotting and exploring 3D contours in `R` with the `rgl` package is quite slow and visualizations do not look very good. We recomend to export the UD probability volumes to visualize them in other 3D visualization software such as `Paraview`.
...
The following picture is a capture from `Paraview` showing the same probability contours as in the previous figure. Red contours correspond to `dentex18`, and green contours to `dentex43`. 50% and the 90% UD contours are represented by the inner and outer volumes, respectively.

\vspace{1em}

![](./3d_paraview.png){width=100%}
