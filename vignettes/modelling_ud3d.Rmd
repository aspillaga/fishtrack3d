...
Some other objects that are created in several steps of this document can be also downloaded in order to follow the method but skipping some steps that might be time consuming.
...
The `syntPath` function generates synthetic path realizations by sampling the geographic coordinates for each detection in the acoustic telemetry data, taking into account the following things:
...
**NOTE: **Running this piece of code is quite slow, if you want to follow this vignette only to see how this method works, one option is to directly load the `depth.cost.list` object from the files downloaded in the beginning of the vignette. Another option is to skip this step and run the `syntPath` function setting the `depth.cost.list` as `NULL`. By doing this, the resulting synthetic paths will avoid emerged land areas, but not the submerged ones.
...
```{r path_plot2D, eval = TRUE, fig.align = "center", fig.width = 3.8, fig.height = 3.5, fig.show = "hold"}
