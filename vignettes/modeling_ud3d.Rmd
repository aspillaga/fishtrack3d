...
All the neccesary functions and a sample dataset are distributed together with the `fishtrack3d` package [GitHub link].
...
```{r plot_bathy, echo = FALSE, fig.align = "center", fig.width = 5.5, fig.height = 5, warnings = FALSE, messages = FALSE}
library(fishtrack3d)
library(raster)
library(plyr)
library(ks)
...
Some additional objects, which are created in some steps during the application of the method, are also provided. Despite we provide all the code to create them, one may want to skip these steps when following this guide, as they are slow and memory consuming or require external software (like `GRASS`).
...
- **`depth_cost_list`:** A list of `TransitionLayer` objects indicating the cost of moving between adjacent raster cells. There is one `TransitionLayer` for each 1 m depth interval (from 0 to 70 m), which considers suitable only cells at depths equal to or greater than the assigned one, and excludes the cells at shallower depths.This object is generated with the `gdistance` package, and is used during the *Generation of random path realizations* (Step 2.2).

- **`synt_path_x100`:** A list containing 100 random path realizations for each of the two common dentex individuals in the sample telemetry data (`tracking` object). This object is created during the *Simulation of synthetic path realizations* (Step 2.2).
...
```{r glm_plot, fig.align = "center", fig.height = 4, fig.width = 5}
...
```{r viewshed_plot, echo = FALSE,  fig.align = "center", fig.height = 8, fig.width = 6.4}
library(raster)
...
We can also make some plots to compare the receivers in two different thinned data frames.

\vspace{0.5em}

```{r spatialCronoPlot, eval = TRUE, echo = TRUE, message = FALSE, fig.align = "center", fig.width = 7, fig.height = 6}
...
We can also see the comparison between depth values from two thinned data frames.

\vspace{1.5em}

```{r depth, eval = TRUE, echo = TRUE, fig.align = "center", fig.width = 7, fig.height = 6}
...
raster::names(depth_cost_list) <- 0:70
...
```{r_get_reconstr_from_simu, eval = TRUE, echo = FALSE}

# To avoid computing the simulations above, we take them directly from the
# provided 'synt_path_x100' object
...
```{r path_plot2D, eval = TRUE, fig.align = "center", fig.width = 5.5, fig.height = 5}
...
rgl.snapshot("./vignettes/3d_traj.png", fmt = "png", top = TRUE)
```


![3D path realizations](./3d_traj.png)
...
**NOTE: **This part of the code can take a long time. To follow this vignette you can skip this step and directly use the `synt_path_x100` object that is provided in the `fishtrack3d` the package.
...
```{r raster_plot, eval = TRUE, echo = FALSE, fig.align = "center", fig.width = 7, fig.height = 4}
...
kde.list <- lapply(rast3d.list, function(r) {
  ...
  depths <- as.numeric(substr(names(r), start = 2, stop = 6))
  
  table <- ldply(1:length(depths), function(d) {
    data.frame(coordinates(r), 
               z = -rep(depths[d], nrow(coordinates(r))),
               val = values(r)[, d])
  
  })
  
  # Kernel density estimation
  t <- table[which(table$val > 0), ]
  kde.tmp <- kde(x = t[, 1:3], w = t$val, compute.cont = FALSE)
...
rgl.snapshot("./vignettes/3d_contour.png", fmt = "png", top = TRUE)
```

![50% and 90% contours (inner and outer volumes) for the two individuals (different color tonalities)](./3d_contour.png)
...
contour.table <- lapply(ud.vol.list, function(r) {
  ...
  # Convert the RasterStack into a table
  depths <- as.numeric(substr(names(r), start = 3, stop = 6))
  
  tmp <- ldply(1:length(depths), function(d) {
...
![Paraview capture showing the same contours probability contours as the previous figure](./3d_paraview.png)
